...
protected:
	...
	virtual void gauQua_normal(BEM& bem, int isFirst, double* data_K,
			double* data_P, double h, double s, double wr, double ws = 0) = 0;
	virtual void gauQua_sin(double* data_K, double* data_P, double h, double s,
			double wr, double ws = 0) = 0;
...
public:
	// ctors/dtor
	Tri3Cell(int nNodNum = 3, int nDim = 3, int nGauNum = GAUPOI,
			int nsinGauNum = SINGAUPOI);
	virtual ~Tri3Cell();
...
private:
	...
	virtual void gauQua_normal(BEM& bem, int isFirst, double* data_K,
			double* data_P, double h, double s, double wr, double ws = 0);
	virtual void gauQua_sin(double* data_K, double* data_P, double h, double s,
			double wr, double ws = 0);
	void gauQua_normal(double* data_K, double* data_P, double h, double s,
			double wr, double ws = 0);
	void gauQua_sur(double* data_S, double h, double s, double wr,
			double ws = 0);
	void surInt();
	void setEleSeq(int, const BEM&);
	//member
