...
//*******************************************************
bgMesh::bgMesh(double* cor, int num):m_dMesInt(BG_INTERVAL)
{
	double min_x = cor[0], min_y = cor[1], min_z = cor[2], max_x = cor[0], max_y = cor[1], max_z = cor[2], dx = 0, dy = 0, dz = 0;
	int count = 0;
	for (int i=0; i<num; i++)
	{
		if (cor[count] < min_x)
...
nodeMF::nodeMF(double* cor,double* fib,double D,double* gcor, int num):node(cor,fib,D),m_mp(4,1)
{
	m_npIInf = new int[NUM_INF];
	for(int i=0; i<NUM_INF; i++)
		m_npIInf[i] = 0;
...
nodeGS::nodeGS():node(),m_mSha(),m_mDB(),m_mP(4,1),m_mA(4,4),m_mB(),m_mInvA(4,4),m_mD(3,3),m_mQ(3,3)
{
...
nodeGS::nodeGS(double* cor, double* wei,nodeMF* mfree, int num):node(cor,wei,0),m_mSha(),m_mDB(),m_mP(4,1),m_mA(4,4),m_mB(),m_mInvA(4,4),m_mD(3,3), m_mQ(3,3)
{
	m_mQ.setData(1.0,0,0);
	m_mQ.setData(1.0,1,1);
	m_mQ.setData(1.0,2,2);
		...
		if (r > 1e-08)
		{
         	  ...
         	  else if( 0.5<r && r<=1 )
    		{
			   ...
			   // w = 4/3 - 4r + 4r^2 - 4/3r^3
    				m_dpWSur[count++] = 4.0/3.0 - 4*r + 4*r*r - 4.0/3.0*r*r*r;
		      // wx = -4rx/r + 8rx - 4rrx 
...
void nodeGS::calSha(nodeMF* node)
{
	...
	for ( int i = 0; i< m_nSur; i++)
	{
		...
		count-=4;
        m_mB.d.setSub(node[m_npISur[i]-1].m_mp*m_dpWSur[count++],0,3,i,i);
		m_mB.dx.setSub(node[m_npISur[i]-1].m_mp*m_dpWSur[count++],0,3,i,i);
		m_mB.dy.setSub(node[m_npISur[i]-1].m_mp*m_dpWSur[count++],0,3,i,i);
		m_mB.dz.setSub(node[m_npISur[i]-1].m_mp*m_dpWSur[count++],0,3,i,i);
	}
	//compute ddSha
...
//	m_mSha.setSub((~m_mP.dz)*m_mInvA.d*m_mB.d.sub(0,3,0,m_nSur-1)+ (~m_mP.d)*m_mInvA.dz*m_mB.d.sub(0,3,0,m_nSur-1) + (~m_mP.d)*m_mInvA.d*m_mB.dz.sub(0,3,0,m_nSur-1),3,3,0,m_nSur-1);
	...
	m_mSha.setSub((~m_mP.dx)*m_mInvA.d*m_mB.d + (~m_mP.d)*m_mInvA.dx*m_mB.d + (~m_mP.d)*m_mInvA.d*m_mB.dx,1,1,0,m_nSur-1);
	m_mSha.setSub((~m_mP.dy)*m_mInvA.d*m_mB.d + (~m_mP.d)*m_mInvA.dy*m_mB.d + (~m_mP.d)*m_mInvA.d*m_mB.dy,2,2,0,m_nSur-1);
	m_mSha.setSub((~m_mP.dz)*m_mInvA.d*m_mB.d+ (~m_mP.d)*m_mInvA.dz*m_mB.d + (~m_mP.d)*m_mInvA.d*m_mB.dz,3,3,0,m_nSur-1);
...
void nodeGS::calFib(nodeMF* mfree)
{
	...
	*/
	...
	double r=sqrt(m_dpFib[0]*m_dpFib[0]+m_dpFib[1]*m_dpFib[1]+m_dpFib[2]*m_dpFib[2]);
...
void nodeGS::calRot()
{
	...
	else
	{
		...
		else if (m_dpFib[0] ==0 && m_dpFib[2] !=0)
		{
		    e1 = m_dpFib[0];
            e2 = m_dpFib[1];
	        e3 = -(m_dpFib[0]*m_dpFib[0] + m_dpFib[1]*m_dpFib[1])/(m_dpFib[2]);
...
void Heart::assTrans()
{
	...
	for(int i=0; i<bg->m_nGPNumAll; i++)
	{
		...
		for(int j=0; j < gspt[i].m_nSur; j++)
			for(int k=0; k<gspt[i].m_nSur; k++)
				m_dpM[m_nMFNum*(gspt[i].m_npISur[k]-1)+gspt[i].m_npISur[j]-1]+= wei*ptr[j]*ptr[k];//wei*ptr[4*j]*ptr[4*k]
	}
	...
	for(int i=0; i<bg->m_nGPNumAll; i++)
	{
		...
		for(int j=0; j < gspt[i].m_nSur; j++)
			for(int k=0; k<gspt[i].m_nSur; k++)
				m_dpK[m_nMFNum*(gspt[i].m_npISur[j]-1)+gspt[i].m_npISur[k]-1]+= ptr[count++];
	}		
}
...
void Heart::checkGP(char* testPath)
{
	...
	::writeFile(file,bg->m_dpGPCor,sizeof(double),DIM*bg->m_nGPNumAll,"wb");
...
//ctors/dtor
//**********************************
Simulator::Simulator(char* path):U(ID,1), V(ID,1), T(ID,ID),H(YDIM,ID),Sti_all(),Sti(ID,1),Par(ID,1),state(),mea(),u0_RK(ID,1),v0_RK(ID,1),ui_RK(ID,1),vi_RK(ID,1),
                                 u0_fg(ID,1),v0_fg(ID,1),vec_one(ID,1)
{	
	char sid[5];
	sprintf(sid,"%d",ID);
...
void Simulator::setExcitation(char* path)
{
	...
	cin>>duration_exc;
	cout<<"Input the delay of RV excitation relative to LV: (-1: RBBB; -2: LBBB; >=0, no block"<<endl;
	cin>>t_delay;
	if (t_delay == -2)
    {
		step_delay = -2;
//		genInitRV();
 ...
 //   	genInitLV();
    ...
    else
    {
    	step_delay = 0;
    	if (t)
    		for(step_delay = 0; step_delay<step_simul && t[step_delay]<t_delay; step_delay++)	{}
    	else
 ...
 //  		genInitLV();
    	...
    	else
    	{
    		for (int i=0; i<=round(duration_exc/dt_simul); i++)
   	    	    sti_l[i] = 1;  		
    		for (int i=step_delay; i<=step_delay+round(duration_exc/dt_simul); i++)
        	    sti_r[i] = 1;
    	}
   	}
//	engine.addData(T,"C");	
...
//		processA('P');
		...
		else
		{
			while (pos < step_simul-1)
    		{
    		    ...
    		    {
    		    	for(int i=pos; i<= pos + round(duration_focus/dt_simul); i++)
    		    		sti_p[i] = 1;
    		    }
    		    pos = pos + round(duration_focus/dt_simul) + round((1.0/freq_focus*1000)/dt_simul);
    		 }
		}
	}	
...
void Simulator::fg(Matrix& u, Matrix& v)
{
	...
	if (PAR_TYPE=='a')
	{
		...
		{
		    u = (u0_fg&(vec_one-u0_fg)&(u0_fg-Par))*PAR_C1 - (u0_fg&v0_fg)*PAR_C2+ Sti;
	       v = (u0_fg - v0_fg*PAR_DM)*PAR_BM;
		}
	    else
		{
		    u = (u0_fg&(vec_one-u0_fg)&(u0_fg-Par))*PAR_K - (u0_fg&v0_fg) + Sti;
	...
	else
	{
		if(FHN_TYPE == 'O')
		{
			u =  (u0_fg&(vec_one-u0_fg)&(u0_fg-vec_one*PAR_AO)) - v0_fg + Sti;
	        v = Par&(u0_fg - v0_fg*PAR_DO);
		}
	    else if(FHN_TYPE == 'M')
		{
		    u = (u0_fg&(vec_one-u0_fg)&(u0_fg-vec_one*PAR_AM))*PAR_C1 - (u0_fg&v0_fg)*PAR_C2 + Sti;
	        v = Par&(u0_fg - v0_fg*PAR_DM);
		}
	    else
		{
		    u = (u0_fg&(vec_one-u0_fg)&(u0_fg-vec_one*Par))*PAR_K- (u0_fg&v0_fg) + Sti;
	        v = Par&((u0_fg&(u0_fg - vec_one*(Par+vec_one)))*PAR_K + v0_fg)*(-1);
		}
	}
...
double Simulator::numDiff(double* head_u, double* head_t, char type)
{
	if (type=='C')   // f(1) - f(-1) / dt; head on -1
		...
		return ( -3*head_u[0] + 4*head_u[1] - head_u[2] )/(head_t[2] - head_t[0]);
	else  // 3f(0) - 4f(-1) + f(-2)   head on -2
        return ( 3*head_u[2] - 4*head_u[1] + head_u[0] )/(head_t[2] - head_t[0]);
}

//*********************
//get initial pos of possible AT
//**********************
void Simulator::iniAT(int& s_pos, int& f_pos, int& rs_pos, int& rf_pos,int& max_pos,double& max,double* u)
{
	int peak_pos, re_pos;
