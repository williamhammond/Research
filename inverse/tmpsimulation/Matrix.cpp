...
Matrix::Matrix(double* data, int r, int c) :
		mn_row(r), mn_col(c) {
...
//***********************************
//set seveal elements of the matrix to the same value
//************************************
void Matrix::setData(double data, int rs, int rf, int cs, int cf) {
	for (int i = cs; i <= cf; i++)
		for (int j = rs; j <= rf; j++)
			mdp_data[i * mn_row + j] = data;
}
...
void Matrix::operator =(const Matrix& m) {
	if (mn_row != m.getRow() || mn_col != m.getCol()) {
		cout
				<< "Error: matrices on lh and rh of '=' must be agree in dimension!"
				<< endl;
		wait();
		exit(0);
	}
...
void Matrix::operator-=(const Matrix& m) {
	if (m.mn_row != mn_row || m.mn_col != mn_col) {
		cout << "Error: matrix dimension does not agree in substraction!"
				<< endl;
		wait();
	} else {
		for (int i = 0; i < mn_row * mn_col; i++)
...
Matrix Matrix::operator&(const Matrix& m) {
	if (m.mn_row != mn_row || m.mn_col != mn_col) {
		cout << "Error: matrix dimension does not agree in dot multiplication!"
				<< endl;
		wait();
		exit(0);
	} else {
...
Matrix Matrix::operator*(const Matrix& m) {
	if (mn_col != m.mn_row) {
		cout << "Error: matrix dimesion does not agree in multiplication!"
				<< endl;
		wait();
		exit(0);
	} else {
		...
		// A is 1*c vector
		if (mn_row == 1)
			vecLTimesMat(matrix.mdp_data, mdp_data, m.mdp_data, m.mn_row,
					m.mn_col);
		else if (m.mn_col == 1)
...
Matrix Matrix::operator-(const Matrix& m) {
	if (mn_row != m.mn_row || mn_col != m.mn_col) {
		cout << "Error: matrix dimesion does not agree in substraction!"
				<< endl;
		wait();
		exit(0);
	} else {
...
Matrix Matrix::inv_gsl() {
	gsl_matrix* m = gsl_matrix_calloc(mn_row, mn_col);
	int count = 0;
	for (int i = 0; i < mn_row; i++)
		for (int j = 0; j < mn_col; j++)
			gsl_matrix_set(m, j, i, mdp_data[count++]);

	gsl_matrix *invm = gsl_matrix_calloc(mn_row, mn_col);
	gsl_permutation *p = gsl_permutation_alloc(mn_row);
	int s = 0;
	gsl_linalg_LU_decomp(m, p, &s);
	gsl_linalg_LU_invert(m, p, invm);

	Matrix m_invm(mn_row, mn_col);
	for (int i = 0; i < mn_row; i++)
		for (int j = 0; j < mn_col; j++)
			m_invm.setData(gsl_matrix_get(invm, i, j), i, j);
...
//********************************************
//check whether the matrix has NAN or INF
//*******************************************
void Matrix::checkNanInf(char* name) {
	int num = mn_row * mn_col;
	for (int i = 0; i < num; i++) {
		if (isnan(mdp_data[i])) {
			cout << "nan happen @" << name << endl;
			wait();
		}
	}
	for (int i = 0; i < num; i++) {
		if (isinf(mdp_data[i])) {
			cout << "inf happen @" << name << endl;
			wait();
		}
	}
}
...
//*******************************************************
void Matrix::vecLTimesMat(double* ans, double* vec, double* mat, int row,
		int col) {
	::reSet(ans, col);
	int count = 0;
	for (int i = 0; i < col; i++)
...
//*******************************************************
void Matrix::vecRTimesMat(double* ans, double* vec, double* mat, int row,
		int col) {
	::reSet(ans, row);
	int count = 0;
	for (int i = 0; i < col; i++)
