...
//*********************************************************
void FECG(char** filNamBEM, char** filNamMFree, char** filOutput, char** TransOutput)
{
...
//**********************************************************
void FECG(char** filNamBEM, char** filNamMFree, char** filOutMFree, char** filOutBEM,char** TransOutput)
{
	...
	engine->evalString("I = horzcat(zeros(numSur_t - numSur_h,numSur_h),eye(numSur_t - numSur_h))");
	engine->evalString("P = vertcat(P,I)");
	
	engine->evalString("P = P*inv(Pt)*Kt");

	engine->evalString("Trans = inv(K+P)*vertcat(M,zeros(numSur_t - numSur_h,numVol))");
...
int computeFECG()
{
	...
	do{
    	int type;
    	cout<<"select the forumlation: 1, without heart surface; 2, with heart surface"<<endl;
        cin>>type;
    	int num_file = 0;
       //ReadMe: filNamBEM**********************************************************
    	...
    	if (type == 1 || type == 2)
    	{
	        ...
	        namMFree[1] = "heart.fib";//"sphFib_mfree_1388.bin";// "auckland_1720.fib"; //
...
void testFECG()
{
	...
	cout<<"1, single elment integral;   2, whole surface integral;   3, MFree integral"<<endl;
	cin>>type;
	if (type==1)
	{
