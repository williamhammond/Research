...
//	cout<<"Save gpts"<<endl;
	...
	mfree.m_mpGPshaD[mfree.m_nGPNum] = D*(shape.sub(0,2,0,m_nSur-1))*m_dWei;
	
	mfree.m_mpGPsha[mfree.m_nGPNum].setData(0.0,4,m_nSur);
	mfree.m_mpGPsha[mfree.m_nGPNum].setSub(sha.d.sub(0,0,0,m_nSur-1),0,0,0,m_nSur-1);
	mfree.m_mpGPsha[mfree.m_nGPNum].setSub(shape.sub(0,2,0,m_nSur-1),1,3,0,m_nSur-1);
...
//*******************************************************
MFree::MFree(double mesInt,char** filNam,BEM& bem):m_nMesInt(mesInt)
{
	//read in data-------------------------------------------------------------
	//nodal cor
...
//	m_dpGPsha = new double[MAX_GP*MAX_INF];
	...
	m_dpTrans_arranged = new double[m_nNodNum*(bem.m_nNodNum+1-bem.m_nForNum)];
...
//	double* con = NULL;
	...
	if (c == 'y' || c == 'Y')
	{
		...
		if(!FIBER_INTER && !FIBER_CLOSE)
		{
			int tempt;
        	int pos = 0;
        	readFile("../BEMForward/GP/gp_true.fib",&data,sizeof(double),3,tempt);
	...
	double* ptr = NULL;
	cout<<"*****transfer matrix assemble**************************************"<<endl;
	for(j=1; j<=bem.m_nNodNum; j++)
	{
		if (!bem.isForbidden(j))
		{
            ...
            for ( l = 0; l < m_nGPNum; l++)
			{
			   ...
			   {
				  m_dpTrans[pos_bem+m_npGPNB[pos_sur]-1] += m_dpR[0]*ptr[pos_gp]+m_dpR[1]*ptr[pos_gp+1]+m_dpR[2]*ptr[pos_gp+2];
				  pos_sur++;
				  pos_gp += 3;
			   }
			}
...
//	test("MFree_original.bin",m_dpTrans,sizeof(double),m_nNodNum*(bem.m_nNodNum+1),"wb");
    ...
    writeFile(output,m_dpTrans_arranged,sizeof(double),m_nNodNum*(bem.m_nNodNum+1-bem.m_nForNum),"wb");
    f = clock();
	d = double(f-s)/CLOCKS_PER_SEC;
