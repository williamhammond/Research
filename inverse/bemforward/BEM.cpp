...
BEM::BEM( char** filNamBEM, char** filNamMFree,CellType celTyp)
{
	...
	m_npNodSeq = NULL;	
	m_npForbidden = NULL;
	readFile(filNamBEM[0],&m_dpNodCor,sizeof(double),m_ptrBEMEle->m_nDim,m_nNodNum);
	readFile(filNamBEM[1],&m_npNodSeq,sizeof(int),m_ptrBEMEle->m_nNodNum,m_nEleNum);
	if(FORBIDDEN)
...
//		m_dpRefCor[j] = 0;
	...
	if(m_nForNum)
	{
	   ...
	   {
		   cout<<"Error in determing discarded elements!"<<endl;
	    	exit(0);
	   }
	   }
	else
...
//************************************************************************************************************************
//ctor/dtor
BEM_source::BEM_source(char** filNamBEM,char**filNamMFree,CellType type):BEM(filNamBEM,filNamMFree,type)
{
	m_dpRefCor = new double[m_ptrBEMEle->m_nDim];
	for(int j=0; j<m_ptrBEMEle->m_nDim; j++)
		m_dpRefCor[j] = 0;
	...
	m_dpTetCor = NULL;
	m_npTet = NULL;	
	readFile(filNamBEM[4],&m_dpTetCor,sizeof(double),m_ptrBEMEle->m_nDim,m_nTetNodNum);
...
int BEM_source::isOutward()
{
	if( OUT_BOUNDARY == 'T')
	{
		if(!IS_CONVEX)
		{
			varCor.x = m_ptrBEMEle->m_dpNodCor[0] - SCALE_T*m_ptrBEMEle->m_dpNor[0];
    		varCor.y = m_ptrBEMEle->m_dpNodCor[1] - SCALE_T*m_ptrBEMEle->m_dpNor[1];
    		varCor.z = m_ptrBEMEle->m_dpNodCor[2] - SCALE_T*m_ptrBEMEle->m_dpNor[2];	
		...
		//not always right, especially in neck, arm region
		{
    		...
    		if( dx*m_ptrBEMEle->m_dpNor[0] +  dy*m_ptrBEMEle->m_dpNor[1] +  dz*m_ptrBEMEle->m_dpNor[2] > 0)
    			return 1;
    	    else
    			return 0;
		}
...
BEM_nosource::BEM_nosource(char** filNamBEM,char** filNamMFree, CellType type)
{
	...
	int* seq_inner = NULL, *seq_outer = NULL,*for_inner = NULL,*for_outer = NULL;
	readFile(filNamBEM[0],&cor_inner,sizeof(double),m_ptrBEMEle->m_nDim,m_nNodNumInner);
	readFile(filNamBEM[1],&seq_inner,sizeof(int),m_ptrBEMEle->m_nNodNum,m_nEleNumInner);
	if(FORBIDDEN)
		readFile(filNamBEM[2],&for_inner,sizeof(int),1,m_nForNumInner);
	else
		m_nForNumInner = 0;
	readFile(filNamBEM[4],&cor_outer,sizeof(double),m_ptrBEMEle->m_nDim,m_nNodNumOuter);
	readFile(filNamBEM[5],&seq_outer,sizeof(int),m_ptrBEMEle->m_nNodNum,m_nEleNumOuter);	
	if(FORBIDDEN)
	...
	writeFile(filNamBEM[7],m_dpNodCor,sizeof(double),m_ptrBEMEle->m_nDim*m_nNodNum,"wb");
	writeFile(filNamBEM[8],m_npNodSeq,sizeof(int),m_ptrBEMEle->m_nNodNum*m_nEleNum,"wb");
	if (m_npForbidden)
